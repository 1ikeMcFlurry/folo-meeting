...
**一台随身录音器，一个 Android App，把现场对话整理成可编辑、可归档的会议纪要。**
...
Folo Meeting 将 **录音器固件、Android App、协议实现和构建工具** 放在一个仓库中，配套发布和追踪版本。录音器通过 Wi-Fi 直接向听悟发送声音；会后在手机上获取转写和 AI 纪要，确认发言人姓名、编辑内容，再发布到自己的飞书账号。

当前为 **v0.1.0 试用版**，已经完成短录音、配网、会后声纹候选和纪要编辑的真机验证。长会议稳定性、多人声纹准确率仍需继续验证，已知限制见本文后半部分。

| 能力 | 当前实现 |
| --- | --- |
| 独立录音 | ESP32-C3 + ES8311 麦克风，设备确定键开始 / 结束，Wi-Fi 直传听悟 |
| BLE 配网 | 乐鑫官方配网组件，扫描设备附近的 2.4 GHz Wi-Fi，分步显示连接结果 |
| AI 纪要 | 听悟完成转写、说话人分离、纪要与待办等任务输出 |
| 会后编辑 | 云端原稿与手机编辑稿分开保存，可调整标题、纪要、逐字稿和本场姓名 |
| 声纹匹配 | 讯飞声纹注册与测试；会后取样匹配、手动确认候选、补录后独立验证 |
...
| 文件 | 用途 |
| --- | --- |
| `folo-meeting-v0.1.0-android.apk` | 安装到 Android 8.0 及以上手机 |
| `folo-meeting-v0.1.0-esp32c3-app.bin` | 已有兼容分区布局的 **8 MB ESP32-C3 工牌**应用升级包 |
...
| 需要准备 | 说明 |
| --- | --- |
| 兼容录音器 | ESP32-C3、8 MB Flash、ST7789P3 屏幕、ES8311 音频、ADC 三键；[引脚定义](components/platform/platform_esp32/include/platform/board_config.h) |
| Android 手机 | Android 8.0+、BLE、可访问云服务的网络 |
| Wi-Fi | 录音器可连接的 2.4 GHz 网络，设备需要独立访问互联网 |
...
2. 长按录音器 **上键** 开启蓝牙，在 App「设备」页点击 **搜索附近设备**，选择 `Folo-Meeting-xxxx`。
3. 首次连接时，在系统配对提示中输入设备屏幕显示的六位码。
4. 点击 **配置 Wi-Fi**，选择录音器扫描到的网络，输入密码。也可以手动添加隐藏网络。
...
保持设备连接，点击 **保存并发送给录音器**；也可以回到「设备」页点击 **发送设置给录音器**。App 保存成功只代表手机有配置，必须发送并核对设备状态后，录音器才会使用这些设置。

### 4. 录制第一场会议

1. 按录音器 **确定键** 开始录音，也可在 App 设备页发起。
...
设备自动保存需要额外点击 **授权录音器自动保存**，两端授权独立。要先确认声纹姓名再发布，请使用「手机确认后发布」流程。
...
> **注册声纹不会直接把听悟的“发言人 1、2、3”变成姓名。** 听悟分离的是本场说话人；App 还要取样、调用讯飞匹配，并由用户确认。已手动命名的发言人会保留原名。
...
| 服务配置 | 用户在 App 中填写；手机通过 Android Keystore 支持的加密存储保存，录音器所需配置写入设备 NVS |
