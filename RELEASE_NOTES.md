...
现阶段先接入 **阿里云听悟**与**讯飞声纹识别（新版）**。听悟可以区分一场会议中的不同发言人，输出“发言人 1、2……”、逐字稿和 AI 纪要；讯飞负责声纹库的创建、录入和管理，以及声音片段与已登记人员的声纹匹配。App 显示匹配得到的姓名候选，经用户确认后应用到本场编辑稿。

不配置讯飞也能通过听悟区分发言人、获取纪要并手动命名；需要通过声纹库匹配具体人员时，再配置讯飞。飞书用于可选的文档归档。
